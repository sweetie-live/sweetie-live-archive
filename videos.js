/* ============================================================
   動画データ:admin.html で生成して丸ごと置き換える
   id / date / title / tour / venue / setlist / tweet / flyer
   ============================================================ */
const VIDEOS = [
  {
    id: "bUhmeWKAhBw",
    date: "2026-08-22",
    title: "箕面の森の音楽会",
    tour: "",
    venue: "箕面公園",
    setlist: ["スジャータ", "だからずっとドキドキワクワク", "恋バナ", "スピリチュアルにおねがい", "いい夢みてね", "なみだなみだだよ", "スジャータ"],
    tweet: "https://x.com/sweetie_genki/status/2090730589469233215?s=46",
    flyer: "flyers/2026-08-22.jpeg"
  },
  {
    id: "qnuBuRXWd6w",
    date: "2026-06-20",
    title: "やまぴーpre.「アイス将軍vol.15」",
    tour: "",
    venue: "八王子Match Vox",
    setlist: ["だからずっとドキドキワクワク", "ミックスソフトクリーム", "スジャータ", "恋バナ", "いい夢みてね", "スピリチュアルにおねがい", "なみだなみだだよ"],
    tweet: "https://x.com/sweetie_genki/status/2048060985173586291",
    flyer: "flyers/2026-06-20.jpg"
  },
  {
    id: "6dWBbJJGURU",
    date: "2026-06-04",
    title: "Etranger “Kishibe” Release TOUR 2026",
    tour: "",
    venue: "堺FANDANGO",
    setlist: ["スジャータ", "だからずっとドキドキワクワク", "スジャータ", "恋バナ", "なみだなみだだよ", "いい夢みてね"],
    tweet: "https://x.com/fandango_sakai/status/2061752948821221690",
    flyer: "flyers/2026-06-04.jpg"
  }
];
